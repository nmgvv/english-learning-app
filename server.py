...
async def api_session_submit(
    ...
    # 规范化：去首尾空格/标点，多个空格合并为一个，忽略连字符
    user_input_normalized = ' '.join(data.input.strip().strip('.,!?;:').lower().split()).replace('-', '')
    word_normalized = ' '.join(data.word.strip('.,!?;:').lower().split()).replace('-', '')
