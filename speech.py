...
# ==================== 阿里云百炼 Qwen3-TTS 语音合成 ====================

class Qwen3TTS:
    """
    阿里云百炼 Qwen3-TTS-Flash 语音合成器

    特点：
    - 智能语调：根据文本内容自动调整语调（疑问句升调、感叹句激情等）
    - 49种音色：支持多种语言和方言
    - 低延迟：97ms 首包延迟
    - 高质量：24kHz 采样率
    ...
    使用方法：
        tts = Qwen3TTS()
        audio_data = await tts.synthesize("Hello, how are you?")
        # 保存或返回 audio_data (bytes)
    """

    # 推荐音色
    VOICES = {
        "cherry": "Cherry",      # 活泼年轻女性（推荐）
        "serena": "Serena",      # 温柔女性
        "ethan": "Ethan",        # 温暖男性
        "chelsie": "Chelsie",    # 动漫风格
    }

    def __init__(self):
        self.api_key = os.getenv("DASHSCOPE_API_KEY")
        self.model = "qwen3-tts-flash"
        # 默认使用 Cherry 音色（活泼年轻女性，适合英语教学）
    ...
    async def synthesize(
        ...
        Args:
            text: 要合成的文本
            voice: 音色名称（可选，默认 Cherry）
            language: 语言类型 Chinese/English/Auto（可选，自动检测）
            rate: 语速倍率 0.5-2.0（注意：非流式 API 可能不支持）

        Returns:
            WAV 音频字节，失败返回 None
        """
        if not self.is_available():
            print("[Qwen3-TTS] API Key 未配置")
            return None

        if not text or not text.strip():
            print("[Qwen3-TTS] 文本为空")
            return None
        ...
        try:
            ...
            # 在线程池中运行同步调用（dashscope SDK 是同步的）
            loop = asyncio.get_event_loop()
            ...
            if response.status_code == 200:
                ...
                print(f"[Qwen3-TTS] 响应中无音频数据: {output}")
                return None
            else:
                print(f"[Qwen3-TTS] API 错误: {response.code} - {response.message}")
                return None

        except ImportError:
            print("[Qwen3-TTS] dashscope SDK 未安装")
            return None
        except Exception as e:
            print(f"[Qwen3-TTS] 合成异常: {type(e).__name__}: {e}")
            return None

    async def _download_audio(self, url: str, retries: int = 3) -> Optional[bytes]:
        """从 URL 下载音频，带重试机制"""
        import httpx
            ...
            try:
                async with httpx.AsyncClient(timeout=30.0) as client:
                    ...
                    if response.status_code == 200:
                        ...
                        print(f"[Qwen3-TTS] 下载失败: HTTP {response.status_code}")
            except Exception as e:
                print(f"[Qwen3-TTS] 下载异常 (尝试 {attempt + 1}/{retries}): {e}")
    ...
    async def synthesize_sentences(
        ...
        Args:
            sentences: 句子列表（字符串数组）
            voice: 音色（可选）

        Returns:
            音频字节列表，与 sentences 对应
        """
        audio_list = []

        for sentence in sentences:
            text = sentence if isinstance(sentence, str) else sentence.get("text", "")
...
# 便捷函数
async def synthesize_speech(text: str, voice: str = None, language: str = None) -> Optional[bytes]:
    """语音合成的便捷函数"""
    tts = Qwen3TTS()
    return await tts.synthesize(text, voice, language)
